...
import com.company.springproject.dal.StudentsGroup;
...
public class StudentsGroupController {
    private final CommonRepository<StudentsGroup> repository;

    @PostMapping
    public ResponseEntity<Integer> add(@RequestBody StudentsGroup entity){
        return ResponseEntity.ok(repository.save(entity).Id);
    }

    @GetMapping
    public ResponseEntity<StudentsGroup> get(@RequestParam Integer entityId){
        return ResponseEntity.ok(repository.findById(entityId).orElseThrow());
    }

    @GetMapping("/getAll")
    public ResponseEntity<List<StudentsGroup>> getAll(){
    ...
    @PatchMapping()
    public ResponseEntity<Integer> update(@RequestBody StudentsGroup entity){
